...
export class DashboardController {
  ...
  async getStats(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const { data: highRiskPatients } = await supabaseAdmin
        .from('patients')
      ...
      const followUps = (nextSessions || []).map((s: any) => ({
        initials: s.patients?.full_name ? s.patients.full_name.split(' ').map((n: string) => n[0]).join('').toUpperCase() : 'PT',
      ...
      logger.error('Error in getStats dashboard controller', { error: (err as Error).message });
