...
class PatientController {
    ...
    async createPatient(req, res) {
        try {
            ...
            const { data: patient, error } = await supabase_1.supabaseAdmin
                .from('patients')
                .insert({
                doctor_id: doctorId,
                full_name: fullName,
                ...
                .single();
            if (error || !patient) {
                logger_1.logger.error('Failed to create patient in Supabase', { error: error?.message });
                res.status(500).json({ status: 'error', message: error?.message || 'Failed to create patient' });
                return;
            }
            logger_1.logger.info('Patient created successfully', { patientId: patient.id, phoneNumber: cleanPhone });
            ...
            if (sendWelcome !== false) {
                try {
                    ...
                    logger_1.logger.warn('Failed to send WhatsApp welcome message during patient creation', {
            ...
            logger_1.logger.error('Error in createPatient controller', { error: err.message });
    ...
    async getPatients(req, res) {
        try {
            ...
            if (error) {
                logger_1.logger.error('Failed to fetch patients', { error: error.message });
            ...
            logger_1.logger.error('Error in getPatients controller', { error: err.message });
            res.status(500).json({ status: 'error', message: 'Internal server error' });
        }
    }
    /**
    ...
    async getPatientById(req, res) {
        try {
            ...
            logger_1.logger.error('Error in getPatientById controller', { error: err.message });
            res.status(500).json({ status: 'error', message: 'Internal server error' });
        }
    }
}
